...
describe("Creación de miembros con datos de Mockaroo Apriori Validacion String Inputs", function () {
    it("e2e - Creación de miembros con datos de Mockaroo Apriori Validacion String Inputs", function () {
        cy.visit(`${APP_PAGE}/ghost/#/signin`);
        cy.wait(1000);
        ...
        cy.window().then(async (win) => {
            ...
            // When: Se crean datos pseudo aleatorios
            const { email,maliciousData } = await getPseudoRamdonMemeber();

            // When: El usuario crea un nuevo miembro
            members.getNewMemberButton().click();
            members.getMemberNameInput().type(maliciousData);
            members.getMemberEmailInput().type(email);
            members.getMemberNoteTextarea().type(maliciousData);
...
describe("Creación de miembros con datos de Mockaroo Apriori", function () {
  it("e2e - Creación de miembros con datos de Mockaroo Apriori", function () {
      cy.visit(`${APP_PAGE}/ghost/#/signin`);
      cy.wait(1000);
      ...
      cy.window().then(async (win) => {
          ...
          // When: Se crean datos pseudo aleatorios
          const { email,maliciousData,name } = await getPseudoRamdonMemeber();
