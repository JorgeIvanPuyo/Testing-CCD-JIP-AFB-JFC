...
describe("Gestión de Tags en la Plataforma", function () {
    it("Crear un nuevo tag y asignarlo a un post e2e - datos aleatorios usando 'Faker'", function () {
  ...
  describe("Gestión de Tags en la Plataforma", function () {
    it("Crear un nuevo tag y asignarlo a un post e2e - datos de 'Mockaroo'", function () {
        // Given Un usuario con credenciales válidas
        cy.visit(`${APP_PAGE}/ghost/#/signin`);
        const signinPage = new SigninPage(cy);
        const modifyPostPage = new ModifyPostPage(cy);
        ...
        createTagPage.getTagNameInput().type(tagData.nameTag);
        createTagPage.getTagAccentColorInput().type(tagData.color.substring(1)); // Remover '#' del color
        const tagSlug = faker.lorem.word(); // Generar un slug del nombre
        createTagPage.getTagSlugInput().type(tagSlug);
        createTagPage.getTagDescriptionTextarea().type(tagData.description);
